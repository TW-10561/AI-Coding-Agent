...
export class WorkspaceManager implements vscode.Disposable {
   ...
   *       helpers.ts
   */
  async buildRepoTree(maxDepth = 3, maxFiles = 80): Promise<string> {
    const roots = vscode.workspace.workspaceFolders;
    ...
    const walk = async (uri: vscode.Uri, indent: string, depth: number) => {
  ...
  async getGitStatus(): Promise<{ branch: string; changes: string[] } | null> {
    const roots = vscode.workspace.workspaceFolders;
  ...
  async getGitLog(count = 5): Promise<string | null> {
    const roots = vscode.workspace.workspaceFolders;
    if (!roots || roots.length === 0) return null;
    const cp = await import("child_process");
  ...
  async buildFullContextString(): Promise<string> {
    ...
    try {
      ...
      if (git) {
        ...
        if (git.changes.length > 0) {
          ...
          if (git.changes.length > 15) gitStr += `\n... +${git.changes.length - 15} more`;
        } else {
          gitStr += "\nWorking tree clean";
        }
