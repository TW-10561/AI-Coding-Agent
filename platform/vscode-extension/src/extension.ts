...
export function activate(context: vscode.ExtensionContext) {
  ...
  const baseUrl = config.get<string>("platformUrl", "http://localhost:3100");
  const apiKey = config.get<string>("apiKey", "");

  client = new ThirdwaveClient({ baseUrl, apiKey: apiKey || undefined });
  ...
  chatProvider = new ChatViewProvider(context.extensionUri, client, context, workspaceManager);
  ...
  // ── Commands ───────────────────────────────────────────────────
  context.subscriptions.push(
    vscode.commands.registerCommand("thirdwave.openChat", () => {
    ...
    vscode.commands.registerCommand("thirdwave.deleteSession", async (item) => {
      if (item?.sessionId) {
        const confirm = await vscode.window.showWarningMessage(
          `Delete session "${item.label}"?`, { modal: true }, "Delete"
        );
        if (confirm === "Delete") {
    ...
    vscode.commands.registerCommand("thirdwave.selectModelById", async (modelId: string, modelName: string) => {
      const config = vscode.workspace.getConfiguration("thirdwave");
    ...
    vscode.commands.registerCommand("thirdwave.viewSkill", async (skillId: string, skillName: string) => {
    ...
    vscode.commands.registerCommand("thirdwave.searchSkills", async () => {
      ...
      try {
        ...
        if (pick) {
          await showSkillPanel((pick as any).skillId, pick.label);
        }
      } catch (e: any) {
  ...
  context.subscriptions.push(
    vscode.workspace.onDidChangeConfiguration((e) => {
      if (e.affectsConfiguration("thirdwave")) {
        ...
        client = new ThirdwaveClient({
          baseUrl: cfg.get<string>("platformUrl", "http://localhost:3100"),
...
function updateAgentStatusBar() {
  ...
  const icons: Record<string, string> = { build: "$(tools)", plan: "$(book)", explore: "$(search)", general: "$(lightbulb)" };
...
async function showBudgetPanel() {
  try {
    const budget: any = await client.budgetSummary();
...
async function showAuditPanel() {
  try {
    const logs: any = await client.queryAudit({ limit: 30 });
...
async function showPoliciesPanel() {
  try {
    const pol: any = await client.policyStatus();
...
async function showSkillPanel(skillId: string, skillName: string) {
  try {
