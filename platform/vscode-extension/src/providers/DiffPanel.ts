...
export class DiffPanel {
  private static _current: DiffPanel | undefined

  private readonly _panel: vscode.WebviewPanel
  private readonly _opts: DiffOptions
  private _disposed = false

  private constructor(panel: vscode.WebviewPanel, opts: DiffOptions) {
    this._panel = panel
    this._opts = opts
  ...
  static show(
    ...
    const panel = vscode.window.createWebviewPanel(
      "thirdwave.diffPanel",
      `Diff — ${opts.filePath.split("/").pop() ?? "file"}`,
...
function buildDiff(original: string, proposed: string): { leftLines: DiffLine[]; rightLines: DiffLine[] } {
  ...
  for (const [lo, lp] of lcs) {
    while (oi < lo) { leftLines.push({ type: "removed", text: origLines[oi++] }); rightLines.push({ type: "context", text: "" }) }
    while (pi < lp) { leftLines.push({ type: "context", text: "" }); rightLines.push({ type: "added", text: propLines[pi++] }) }
    leftLines.push({ type: "context", text: origLines[oi++] })
    rightLines.push({ type: "context", text: propLines[pi++] })
  }
  while (oi < origLines.length) { leftLines.push({ type: "removed", text: origLines[oi++] }); rightLines.push({ type: "context", text: "" }) }
  while (pi < propLines.length) { leftLines.push({ type: "context", text: "" }); rightLines.push({ type: "added", text: propLines[pi++] }) }
