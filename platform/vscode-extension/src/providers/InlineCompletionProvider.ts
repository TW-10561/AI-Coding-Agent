...
export class InlineCompletionProvider implements vscode.InlineCompletionItemProvider {
  ...
  async provideInlineCompletionItems(
    ...
    if (!cfg.get<boolean>("enableInlineCompletion", true)) return null;

    // Don't trigger if the cursor is at the start or line is too short
    const lineText = document.lineAt(position.line).text;
    const textBeforeCursor = lineText.substring(0, position.character);
    if (textBeforeCursor.trim().length < MIN_TRIGGER_CHARS) return null;
    ...
    if (requestKey === this._lastRequest) return null;
    ...
    return new Promise<vscode.InlineCompletionList | null>((resolve) => {
      this._debounceTimer = setTimeout(async () => {
        ...
        try {
          ...
          const prompt =
            `You are an expert ${lang} developer. Complete the code at the cursor position.\n` +
            `File: ${fileName}\n` +
            `Language: ${lang}\n\n` +
            `<prefix>\n${truncatedPrefix}\n</prefix>\n` +
            (suffix ? `<suffix>\n${suffix}\n</suffix>\n` : "") +
