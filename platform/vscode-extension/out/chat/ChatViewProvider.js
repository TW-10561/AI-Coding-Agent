...
class ChatViewProvider {
    ...
    _deleteSessionHistory(sessionId) {
        const key = `thirdwave.sessionHistory.${sessionId}`;
    ...
    _restoreSessions() {
        return this._context.globalState.get("thirdwave.sessions", []);
    }
    ...
    _ensureToken() {
        const savedToken = this._context.globalState.get("thirdwave.jwt");
        if (savedToken) {
        ...
        webviewView.webview.onDidReceiveMessage(async (msg) => {
            switch (msg.type) {
                ...
                case "login": {
                    ...
                    if (!email || !password) {
                        this._post({ type: "authError", error: "Email and password required" });
                        break;
                    }
                    try {
                        ...
                        const adminVerified = activeKeys.some((k) => k.adminVerified);
                ...
                case "checkAuth": {
                    const savedToken = this._context.globalState.get("thirdwave.jwt");
                    const savedUser = this._context.globalState.get("thirdwave.user");
                    if (savedToken) {
                        this._client.setToken(savedToken);
                        try {
                ...
                case "compactConversation": {
                    ...
                    const compactedMsg = { role: "assistant", content: summary, timestamp: Date.now() };
                ...
                case "ready":
                    ...
                    {
                        const coConfig = this._context.globalState.get("thirdwave.customOpenAI");
                        if (coConfig)
                            this._post({ type: "customOpenAIData", config: coConfig });
                    }
                    break;
                case "setLanguage":
                ...
                case "testCustomOpenAI": {
                    ...
                    const coTest = this._context.globalState.get("thirdwave.customOpenAI", {});
                    ...
                    if (!testUrl) {
                        this._post({ type: "customOpenAITestResult", ok: false, error: "No base URL" });
                        break;
                    }
                    try {
                        const headers = {};
    ...
    async _onUserMessage(text) {
        ...
        try {
            ...
            if (isJapanese) {
                promptParts.push("【重要】あなたはすべての応答を日本語で行わなければなりません。英語で応答しないでください。\n\n" +
                ...
                promptParts.push("You are Thirdwave AI, a friendly and helpful AI coding assistant. " +
            ...
            const agentInstructions = isJapanese ? {
            ...
            if (selectedIds.length > 0) {
                ...
                for (const sid of selectedIds) {
                    try {
                        ...
                        if (sk.content)
                            skillContents.push(`## Skill: ${sk.displayName || sk.name}\n${sk.content}`);
                    }
