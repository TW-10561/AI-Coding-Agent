...
export class AuditLogger {
  ...
  constructor(auditDir?: string) {
    if (auditDir) {
      ...
      try {
        this.auditDir = path.join(Instance.worktree, ".opencode", "audit")
      } catch (error) {
        // Fallback for test environments where Instance context is not available
        this.auditDir = path.join(process.cwd(), ".opencode", "audit")
      }
    }
  }
