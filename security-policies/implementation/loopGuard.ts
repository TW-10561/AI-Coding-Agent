...
export class LoopGuard {
  ...
  /**
   * Get frequency of a command
   */
  getCommandFrequency(command: string): number {
    return this.commandHistory.filter((c) => c.command === command).length
  }

  /**
   * Get statistics about the loop guard state
   */
  getStatistics(): {
    ...
    const uniqueErrors = new Set(this.errorHistory.map((e) => e.error)).size
    const totalAttempts = this.commandHistory.length + this.errorHistory.length
    ...
    return {
      ...
      uniqueCommands,
      uniqueErrors,
      averageErrorRate: totalAttempts > 0 ? this.errorHistory.length / totalAttempts : 0,
    }
  }
}
