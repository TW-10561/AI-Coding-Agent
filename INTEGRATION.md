# Artemis AI Coding Platform — Integration Documentation
...
└──────────────┘               └──────────────┬────────────────────────┘
                                              ...
                                              ▼
                               ┌──────────────────────────────────────┐
                               │   Artemis Platform (Hono, port 3100) │
                               │                                      │
                               │  ┌──────────────────────────────┐   │
                               │  │    Middleware Chain           │   │
...
| **nginx Proxy** | nginx 1.24 | 80 | `platform/deploy/nginx/artemis.conf` | Rate limiting, SSL termination, proxying |
...
| **Ollama Gateway** | Ollama | 11435 | `172.30.140.143:11435` | 5 models (Qwen, GPT-OSS) |
| **CLI Client** | Bash + Python3 | — | `platform/bin/artemis-client` | User-facing terminal tool ("art") |
| **Platform SDK** | TypeScript | — | `platform/src/sdk/client.ts` | Programmatic API client library |
| **systemd Service** | Linux service | — | `platform/deploy/systemd/artemis.service` | Process management + auto-restart |
...
User types: art chat "fix the login bug"
  ...
  CLI (artemis-client)
     ...
     ▼
  nginx (:80)
     │  rate limit: 5r/s (artemis_chat zone)
...
OpenCode is an open-source Go binary that provides AI-powered coding sessions. Artemis uses it as the **session and agent management engine**.
...
When `/api/client` is requested, the server reads `platform/bin/artemis-client`, replaces the default `ARTEMIS_SERVER` value with the actual server URL derived from the request's `Host` header and `X-Forwarded-Proto`:
...
```
ARTEMIS_PORT_OFFSET=0  → Platform :3100, OpenCode :4096  (default)
ARTEMIS_PORT_OFFSET=10 → Platform :3110, OpenCode :4106
ARTEMIS_PORT_OFFSET=20 → Platform :3120, OpenCode :4116
```
...
# Developer B
ARTEMIS_PORT_OFFSET=10 bun run platform/scripts/start-all.ts
# → Platform :3110, OpenCode :4106

# Developer C
ARTEMIS_PORT_OFFSET=20 bun run platform/scripts/start-all.ts
...
| `ARTEMIS_PORT_OFFSET` | `0` | No | Multi-user port shift |
